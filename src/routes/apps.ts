...
// Pending choose custom file requests (re-exported for index.ts compatibility)
    ...
    try {
      ...
      if (app.type === "cli") {
        ...
        const child = spawn("cmd", ["/c", "start", "cmd", "/k",
          "set OPENAI_BASE_URL=" + proxyUrl + "/v1 && set OPENAI_API_KEY=sk-dummy && echo Orca Proxy: " + proxyUrl + "/v1 && echo Provider: " + provider.name
