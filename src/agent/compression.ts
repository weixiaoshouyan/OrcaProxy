...
export async function compressContextIfNeeded(messages: any[], resolved: any): Promise<any[]> {
  ...
  const threshold = Math.max(8000, Math.min(contextLimit, 128000));
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      targetUrl = baseUrl.endsWith("/v1") ? baseUrl + "/chat/completions" : baseUrl + "/v1/chat/completions";
  ...
  const fallbackMsgs = activeMessages.slice(fallbackStart);
