...
export function processChunk(state: StreamState, chunk: Record<string, unknown>): string {
  ...
  // Only surface real content text. reasoning_content is tracked separately
  // (state.reasoningContent) and surfaced in the final completed event — it must
  // NOT be emitted as output_text.delta, otherwise it pollutes the model answer.
